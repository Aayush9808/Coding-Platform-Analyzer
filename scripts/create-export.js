...
export const exportShareLink = (data) => {
  const loading = showToast('Generating link...', 'loading');
  try {
    const usernames = [];
    Object.entries(data.platforms).forEach(([platform, pData]) => {
...
const filePath = path.join(__dirname, 'frontend', 'src', 'utils', 'exportUtils.ts');
fs.writeFileSync(filePath, content, 'utf8');
console.log('✅ Created exportUtils.ts successfully!');
